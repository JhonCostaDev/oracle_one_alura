USE `insight_places`;
DROP procedure IF EXISTS `insight_places`.`novoAluguel_8`;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `novoAluguel_8`(
...
BEGIN
    ...
    IF varNumClientes > 1 THEN		-- não registra aluguel de mais de um cliente tiver o mesmo nome.
		SET varMensagem = 'Este Cliente não pode ser usado para incluir o aluguel pelo nome.';
        SELECT varMensagem;				-- exibe a mensagem 
	ELSEIF varNumClientes = 0 THEN	-- não registra o aluguel se o cliente não for encontrado na base.
		SET varMensagem ='Este cliente não pode ser usado para incluir um novo aluguel porque ele não existe.';
		SELECT varMensagem;				-- exibe a mensagem 
    ELSE 								-- Passados os testes, inclui o aluguel
		...
		SELECT varMensagem;					-- Exibe a mensagem de sucesso.
    END IF;									-- Fim da condicional IF
END$$										-- Fim da Procedure
DELIMITER ;

CALL novoAluguel_8('10007','Victorino Vila','8635','2023-03-30','2023-04-04',40);
